...
| 配置管理 | [config-rs](https://github.com/mehcode/config-rs) + 多环境配置 + 热重载 ([notify](https://github.com/notify-rs/notify)) |
...
├── server/                    # Rust 后端
│   ├── Cargo.toml             # 依赖管理
│   ├── config.yaml            # 默认配置文件（支持多环境：config.{env}.yaml + 热重载）
...
- [x] **系统管理**
  ...
  - [x] 系统配置（获取/修改配置、服务器信息、配置重载、snake_case/kebab-case 自动转换）
  - [x] 错误日志（前端错误自动上报）
  - [x] 邮件发送测试
...
# 推荐：创建环境特定配置文件（已被 .gitignore 忽略，不会提交到仓库）
# system.env 字段值决定加载哪个文件，如 env: local 则加载 config.local.yaml
...
# 通过环境变量指定配置文件（最高优先级）
CONFIG_PATH=config.production.yaml cargo run
```

> **多环境配置**：配置文件加载优先级为 `CONFIG_PATH` 环境变量 > `config.{env}.yaml`（根据 `system.env` 值） > `config.yaml`。环境特定文件 `config.*.yaml` 已被 `.gitignore` 忽略，适合存放本地敏感配置。
...
| 多环境配置 | config.debug/release/test.yaml | config.{env}.yaml（基于 system.env） |
| 配置 key 格式 | kebab-case（YAML + API） | snake_case（YAML）+ API 层自动转 kebab-case |

## 更新日志

### v0.1.0（开发中）

- **基础架构**：Axum + SeaORM + 多环境配置 + 配置热重载 + 日志系统
- **用户认证**：登录 / 注册 / JWT / 验证码 / 密码管理
- **RBAC 权限**：角色 / 菜单 / API / Casbin / 按钮级权限
- **系统管理**：字典 / 操作记录 / 参数 / 系统配置（含 key 格式自动转换） / 邮件 / 错误日志
