<template>
  <div class="h-full">
    <warning-bar href="https://github.com/szyxiii/rust-vue-admin" title="此功能仅在开发阶段使用，用户构建本项目内的skills技能库。" />
    <el-row :gutter="12" class="h-full">
      <el-col :xs="24" :sm="8" :md="6" :lg="5" class="flex flex-col gap-4 h-full">
          ...
          <div class="flex flex-wrap gap-2">
            <div v-for="tool in tools" :key="tool.key"
              ...
              :class="activeTool === tool.key
                ...
                : 'bg-white hover:bg-gray-50 text-gray-700 border-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700 dark:hover:bg-gray-700'" @click="handleToolSelect(tool.key)">
              {{ tool.label }}
            </div>
          </div>
        </el-card>
        ...
        <el-card shadow="never" class="!border-none flex-1 mt-2 flex flex-col min-h-0">
          ...
          <el-input v-model="skillFilter" size="small" clearable placeholder="搜索技能" class="mb-2" prefix-icon="Search" />
            ...
            <el-menu :default-active="activeSkill" class="!border-none" @select="handleSkillSelect">
              <el-menu-item v-for="skill in filteredSkills" :key="skill" :index="skill"
                ...
                <div class="w-full flex items-center justify-between min-w-0">
                  ...
                  <el-button type="danger" link icon="Delete" @click.stop="handleDeleteSkill(skill)">
                    删除
                  </el-button>
                </div>
              </el-menu-item>
              ...
              <el-tab-pane label="技能配置" name="config">
                ...
                <el-form :model="form" label-width="160px">
                  ...
                  <el-form-item>
                    ...
                    <el-input v-model="form.description" placeholder="例如: 为代码添加双语注释，适合代码审查/重构/可读性改进" />
                  ...
                  <el-form-item>
                    ...
                    <div class="mb-2 flex flex-wrap gap-2">
                      <el-button v-for="block in quickBlocks" :key="block.label" size="small"
                        @click="appendMarkdown(block.content)">
                        {{ block.label }}
                      </el-button>
                      <el-button size="small" @click="insertFullTemplate">插入完整模板</el-button>
                    </div>
                    <el-input v-model="form.markdown" type="textarea" :rows="20" :placeholder="markdownPlaceholder" />
                  ...
                  <el-table-column label="操作" width="180">
                    <template #default="scope">
                      <el-button type="primary" link icon="Edit"
                        @click="openScriptEditor(scope.row.name)">编辑</el-button>
              ...
              <el-tab-pane label="资源" name="resources">
                ...
                <el-table :data="resourceRows" style="width: 100%">
                  ...
                  <el-table-column label="操作" width="180">
                    <template #default="scope">
                      <el-button type="primary" link icon="Edit"
                        @click="openResourceEditor(scope.row.name)">编辑</el-button>
                      <el-button type="primary" link
                        @click="insertFileSnippet('resource', scope.row.name)">引用</el-button>
                    </template>
                  </el-table-column>
                </el-table>
              ...
              <el-tab-pane label="References" name="references">
                ...
                <el-table :data="referenceRows" style="width: 100%">
                  ...
                  <el-table-column label="操作" width="180">
                    <template #default="scope">
                      <el-button type="primary" link icon="Edit"
                        @click="openReferenceEditor(scope.row.name)">编辑</el-button>
                      <el-button type="primary" link
                        @click="insertFileSnippet('reference', scope.row.name)">引用</el-button>
                    </template>
                  </el-table-column>
                </el-table>
              ...
              <el-tab-pane label="Templates" name="templates">
                ...
                <el-table :data="templateRows" style="width: 100%">
                  ...
                  <el-table-column label="操作" width="180">
                    <template #default="scope">
                      <el-button type="primary" link icon="Edit"
                        @click="openTemplateEditor(scope.row.name)">编辑</el-button>
                      <el-button type="primary" link
                        @click="insertFileSnippet('template', scope.row.name)">引用</el-button>
                    </template>
                  </el-table-column>
                </el-table>
      ...
      <el-form label-width="90px">
        <el-form-item label="下载到">
          <el-select v-model="downloadTarget" placeholder="请选择工具" class="w-full">
            <el-option v-for="item in downloadTargetOptions" :key="item.value" :label="item.label"
              :value="item.value" />
          </el-select>
        </el-form-item>
    ...
    <el-drawer v-model="editorVisible" size="70%" destroy-on-close :with-header="false">
      <div class="h-full flex flex-col p-4">
        ...
        <div class="flex-1 overflow-hidden border border-gray-200 dark:border-gray-700 rounded-md shadow-inner">
          <v-ace-editor v-model:value="editorContent" :lang="editorLang" theme="github_dark" class="w-full h-full"
    ...
    <!-- 在线 Skills 抽屉 -->
    <el-drawer v-model="onlineDrawerVisible" size="90%" :show-close="false" destroy-on-close>
      <template #header>
      ...
      <div class="mb-4">
        <el-form :inline="true" :model="onlineSearchInfo">
          <el-form-item label="名称">
            <el-input v-model="onlineSearchInfo.name" placeholder="搜索技能名称" clearable
              @keyup.enter="searchOnlineSkills" />
          </el-form-item>
          <el-form-item>
      ...
      <el-table v-loading="onlineLoading" :data="onlineSkillList" stripe>
        <el-table-column label="封面" width="80">
          <template #default="{ row }">
            <el-image v-if="row.picture" :src="row.picture" style="width: 50px; height: 50px" fit="cover"
              class="rounded" />
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="120" fixed="right">
          <template #default="{ row }">
            <el-button v-if="row.money === 0" type="primary" link icon="Download" :loading="downloadingIds.has(row.ID)"
              @click="handleDownloadSkill(row)">下载</el-button>
            <a v-else class="text-blue-500 hover:text-blue-700 text-sm"
              :href="`https://github.com/szyxiii/rust-vue-admin`" target="_blank">去购买</a>
          </template>
        </el-table-column>
      </el-table>
      <div class="flex justify-center mt-4">
        <el-pagination :current-page="onlineSearchInfo.page" :page-size="onlineSearchInfo.pageSize"
          :page-sizes="[10, 20, 50]" :total="onlineTotal" layout="total, sizes, prev, pager, next"
          @current-change="handleOnlinePageChange" @size-change="handleOnlineSizeChange" />
      </div>
    </el-drawer>
  </div>
</template>
...
const pluginMarketLoginURL = 'https://github.com/szyxiii/rust-vue-admin'
