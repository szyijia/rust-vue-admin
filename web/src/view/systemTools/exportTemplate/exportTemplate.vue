<template>
  <div>
    <WarningBar title="本功能提供同步的表格导出功能，大数据量的异步表格导出功能，可以选择点我定制" href="https://github.com/szyijia/rust-vue-admin" />
    <div class="gva-search-box">
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline" :rules="searchRule"
        ...
        <el-form-item label="创建日期" prop="createdAt">
          ...
          <el-date-picker v-model="searchInfo.startCreatedAt" type="datetime" placeholder="开始日期" :disabled-date="(time) =>
              searchInfo.endCreatedAt
                ? time.getTime() > searchInfo.endCreatedAt.getTime()
                : false
            " />
          —
          <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束日期" :disabled-date="(time) =>
    ...
    <div class="gva-table-box">
      ...
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="ID"
        ...
        <el-table-column align="left" label="模板信息" prop="templateInfo" min-width="120" show-overflow-tooltip />
        <el-table-column align="left" label="操作" min-width="280">
          <template #default="scope">
            <el-button type="primary" link icon="documentCopy" class="table-button"
              @click="copyFunc(scope.row)">复制</el-button>
            <el-button type="primary" link icon="edit-pen" class="table-button"
              @click="showCode(scope.row)">代码和SQL预览</el-button>
    ...
    <el-drawer v-model="dialogFormVisible" size="60%" :before-close="closeDialog"
      :title="type === 'create' ? '添加' : '修改'" :show-close="false" destroy-on-close>
      <template #header>
      ...
      <el-form ref="elFormRef" :model="formData" label-position="right" :rules="rule" label-width="100px"
        v-loading="aiLoading" element-loading-text="小淼正在思考...">
        <el-form-item label="业务库" prop="dbName">
          ...
          <el-select v-model="formData.dbName" clearable @change="dbNameChange" placeholder="选择业务库">
        ...
        <el-form-item label="AI帮写:" prop="ai">
          <div class="relative w-full">
            <el-input type="textarea" v-model="prompt" :clearable="true" :rows="5"
              placeholder="试试描述你要做的导出功能让AI帮你完成，在此之前请选择你需要导出的表所在的业务库，如不做选择，则默认使用gva库" />
            <el-button class="absolute bottom-2 right-2" type="primary"
              @click="autoExport"><el-icon><ai-gva /></el-icon>帮写</el-button>
          </div>
        </el-form-item>

        <el-form-item label="表名称:" clearable prop="tableName">
          <div class="w-full flex gap-4">
            <el-select v-model="formData.tableName" class="flex-1" filterable placeholder="请选择表">
              <el-option v-for="item in tableOptions" :key="item.tableName" :label="item.tableName"
                :value="item.tableName" />
            </el-select>
            <el-button :disabled="!formData.tableName" type="primary"
              @click="getColumnFunc(true)"><el-icon><ai-gva /></el-icon>自动补全</el-button>
        ...
        <el-tabs v-model="activeName">
            ...
            <el-form-item label="关联条件:">
              <div v-for="(join, key) in formData.joinTemplate" :key="key" class="flex gap-4 w-full mb-2">
                ...
                <el-button type="danger" icon="delete"
                  @click="() => formData.joinTemplate.splice(key, 1)">删除</el-button>
              </div>
          ...
          <el-tab-pane label="自定义SQL" name="sql" class="pt-2">
            ...
            <el-form-item label="导入SQL:" prop="importSql">
              <el-input v-model="formData.importSql" type="textarea" :rows="10"
    ...
    <!-- 合并：代码模板 + SQL预览 抽屉 -->
    <el-drawer v-model="drawerVisible" size="70%" :title="'模板与预览'" :show-close="true" destroy-on-close>
      <template #header>
        <div class="flex justify-between items-center">
      ...
      <el-tabs v-model="activeTab" type="border-card">
        <el-tab-pane label="代码模板" name="code">
          <v-ace-editor v-model:value="webCode" lang="vue" theme="github_dark" class="w-full h-96"
        ...
        <el-tab-pane label="SQL预览" name="sql">
          <div class="flex flex-col gap-4">
            ...
            <div class="w-full">
              <v-ace-editor v-model:value="previewSQLCode" lang="sql" theme="github_dark" class="w-full h-96"
                :options="aceOptions" />
            </div>
          </div>
        </el-tab-pane>
