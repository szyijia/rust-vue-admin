<template>
  <div>
    <warning-bar href="https://www.bilibili.com/video/BV1kv4y1g7nT?p=3" title="此功能为开发环境使用，不建议发布到生产，具体使用效果请点我观看。" />
    <div class="gva-search-box" v-if="!isAdd">
      <div class="text-lg mb-2 text-gray-600">
        使用AI创建<a class="text-blue-600 text-sm ml-4" href="https://github.com/szyxiii/rust-vue-admin"
          target="_blank">获取AiPath</a>
      </div>
      <div class="relative">
        <el-input v-model="prompt" type="textarea" :rows="5" :maxlength="2000"
          ...
          resize="none" @focus="handleFocus" @blur="handleBlur" />

        <div class="flex absolute right-28 bottom-2">
          <el-tooltip effect="light">
            <template #content>
              <div>
                【完全免费】前往<a class="text-blue-600" href="https://github.com/szyxiii/rust-vue-admin"
                  target="_blank">项目主页</a>获取AIPath，填入config.yaml的ai-path属性即可使用。
              </div>
            </template>
        ...
        <el-row class="w-full">
          <el-col :span="6">
            <el-form-item label="业务库" prop="selectDBtype" class="w-full">
              ...
              <el-select v-model="dbform.businessDB" clearable placeholder="选择业务库" @change="getDbFunc" class="w-full">
          ...
          <el-col :span="6">
            <el-form-item label="数据库名" prop="structName" class="w-full">
              <el-select v-model="dbform.dbName" clearable filterable placeholder="请选择数据库" class="w-full"
                @change="getTableFunc">
          ...
          <el-col :span="6">
            <el-form-item label="表名" prop="structName" class="w-full">
              <el-select v-model="dbform.tableName" :disabled="!dbform.dbName" class="w-full" filterable
                placeholder="请选择表">
    ...
    <div class="gva-search-box">
      ...
      <el-form :disabled="isAdd" ref="autoCodeForm" :rules="rules" :model="form" label-width="120px" :inline="true">
        <el-row class="w-full">
          <el-col :span="6">
        ...
        <el-row class="w-full">
          ...
          <el-col :span="6">
            <el-form-item label="业务库" prop="businessDB" class="w-full">
              ...
              <el-select v-model="form.businessDB" clearable placeholder="选择业务库" class="w-full">
                <el-option v-for="item in dbList" :key="item.aliasName" :value="item.aliasName" :label="item.aliasName"
                  :disabled="item.disable">
                  <div>
                    <span>{{ item.aliasName }}</span>
        ...
        <el-collapse-item>
          ...
          <div class="p-4">
            ...
            <div class="border-b border-gray-200 last:border-0">
              ...
              <el-row :gutter="20">
                <el-col :span="3">
                  <el-tooltip content="注：会自动在结构体global.Model其中包含主键和软删除相关操作配置" placement="top" effect="light">
                ...
                <el-col :span="3">
                  <el-tooltip content="注：会自动产生页面内的按钮权限配置，若不在角色管理中进行按钮分配则按钮不可见" placement="top" effect="light">
            ...
            <div class="border-b border-gray-200 last:border-0">
              ...
              <el-row :gutter="20">
                <el-col :span="3">
                  <el-tooltip content="注：把自动生成的API注册进数据库" placement="top" effect="light">
                ...
                <el-col :span="3">
                  <el-tooltip content="注：把自动生成的菜单注册进数据库" placement="top" effect="light">
                ...
                <el-col :span="3">
                  <el-tooltip content="注：自动同步数据库表结构，如果不需要可以选择关闭" placement="top" effect="light">
            ...
            <div class="border-b border-gray-200 last:border-0">
              ...
              <el-row :gutter="20">
                <el-col :span="3">
                  <el-tooltip content="注：会自动在结构体添加 created_by updated_by deleted_by，方便用户进行资源权限控制" placement="top"
                    effect="light">
                ...
                <el-col :span="3">
                  <el-tooltip content="注：使用基础模板将不会生成任何结构体和CURD,仅仅配置enter等属性方便自行开发非CURD逻辑" placement="top"
                    effect="light">
                ...
                <el-col :span="24">
                  <el-form-item label="树型结构">
                    <div class="flex items-center gap-4">
                      <el-tooltip content="注：会自动创建parentID来进行父子关系关联,仅支持主键为int类型" placement="top" effect="light">
                        <el-checkbox v-model="form.isTree" />
                      </el-tooltip>
                      <el-input v-model="form.treeJson" :disabled="!form.isTree" placeholder="前端展示json属性"
                        class="flex-1" />
                    </div>
                  </el-form-item>
                </el-col>
    ...
    <div class="gva-table-box">
      ...
      <div class="draggable">
        <el-table :data="form.fields" row-key="fieldName">
          ...
          <el-table-column align="left" prop="fieldType" label="字段类型" width="160">
            <template #default="{ row }">
              <el-select v-model="row.fieldType" style="width: 100%" placeholder="请选择字段类型" :disabled="row.disabled"
                clearable>
          ...
          <el-table-column align="left" prop="fieldIndexType" label="索引类型" width="160">
            <template #default="{ row }">
              <el-select v-model="row.fieldIndexType" style="width: 100%" placeholder="请选择字段索引类型"
                :disabled="row.disabled" clearable>
                <el-option v-for="item in typeIndexOptions" :key="item.value" :label="item.label" :value="item.value" />
              </el-select>
            </template>
          </el-table-column>
          <el-table-column align="left" prop="dataTypeLong" label="字段长度/枚举值" width="160">
            <template #default="{ row }">
          ...
          <el-table-column align="left" label="操作" width="300" fixed="right">
            <template #default="scope">
              <el-button v-if="!scope.row.disabled" type="primary" link icon="edit" @click="editAndAddField(scope.row)">
                高级编辑
              </el-button>
              <el-button v-if="!scope.row.disabled" type="primary" link icon="delete"
      ...
      <div class="gva-btn-list justify-end mt-4">
        ...
        <el-upload class="flex items-center" :before-upload="importJson" :show-file-list="false"
          :headers="{ 'x-token': token }" accept=".json">
    ...
    <el-drawer v-model="previewFlag" size="80%" :show-close="false">
      ...
      <PreviewCodeDialog v-if="previewFlag" :is-add="isAdd" ref="previewNode" :preview-code="preViewCode" />
    </el-drawer>
  </div>
</template>
