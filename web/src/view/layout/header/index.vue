<template>
  <div
    ...
    <div class="flex items-center cursor-pointer flex-1">
      <div class="flex items-center justify-center cursor-pointer" :class="isMobile ? '' : 'min-w-48'"
        @click="router.push({ path: '/' })">
        <Logo />
        <div v-if="!isMobile" class="inline-flex font-bold text-2xl ml-2" :class="(config.side_mode === 'head' ||
            config.side_mode === 'combination') &&
          'min-w-fit'
          ">
          {{ $RUST_VUE_ADMIN.appName }}
        </div>
      </div>
