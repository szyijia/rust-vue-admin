  ...
  <div>
      ...
      <el-table :data="tableData" row-key="ID">
        ...
        <el-table-column align="left" fixed="right" label="操作" :min-width="appStore.operateMinWith">
          <template #default="scope">
            ...
            <el-button type="primary" link icon="user" @click="openAssignRoleDrawer(scope.row)">
              分配角色
            </el-button>
    ...
    <el-drawer v-model="dialogFormVisible" :size="appStore.drawerSize" :before-close="handleClose" :show-close="false">
      ...
      <div class="border-b border-gray-200">
        ...
        <el-form v-if="dialogFormVisible" ref="menuForm" :inline="true" :model="form" :rules="rules"
          label-position="top">
          <el-row class="w-full">
            <el-col :span="24">
          ...
          <el-row class="w-full">
            <el-col :span="12">
              <el-form-item label="展示名称" prop="meta.title">
                <el-input v-model="form.meta.title" autocomplete="off" placeholder="请输入菜单展示名称" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
              ...
              <el-form-item prop="path">
                <template #label>
                  <div class="inline-flex items-center h-4">
                    <span>路由Path</span>
                    <el-checkbox class="ml-2" v-model="checkFlag">添加参数</el-checkbox>
                  </div>
                </template>
          ...
          <el-row class="w-full">
            ...
            <el-col :span="8">
              <el-form-item label="排序标记" prop="sort">
                <el-input v-model.number="form.sort" autocomplete="off" placeholder="请输入排序数字" />
              </el-form-item>
            </el-col>
            <el-col :span="8">
              ...
              <el-form-item prop="meta.activeName">
                <template #label>
                  <div class="label-with-tooltip">
                    <span>高亮菜单</span>
                    <el-tooltip content="注：当到达此路由时候，指定左侧菜单指定name会处于活跃状态（亮起），可为空，为空则为本路由Name。" placement="top"
                      effect="light">
                      <el-icon>
                        <QuestionFilled />
                      </el-icon>
                ...
                <el-input v-model="form.meta.activeName" :placeholder="form.name || '请输入高亮菜单名称'" autocomplete="off" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
            ...
            <el-col :span="8">
              <el-form-item>
                <template #label>
                  <div class="label-with-tooltip">
                    <span>路由切换动画</span>
                    <el-tooltip content="如果设置了路由切换动画，在本路由下的动画优先级高于全局动画切换优先级。" placement="top" effect="light">
                      <el-icon>
                        <QuestionFilled />
                      </el-icon>
                    </el-tooltip>
                ...
                <el-select v-model="form.meta.transitionType" style="width: 100%" placeholder="跟随全局" clearable>
      ...
      <div class="border-b border-gray-200">
        ...
        <el-table :data="form.parameters" style="width: 100%" class="parameter-table">
          <el-table-column align="center" prop="type" label="参数类型" width="150">
            <template #default="scope">
          ...
          <el-table-column align="center" prop="key" label="参数key" width="150">
            <template #default="scope">
              <el-input v-model="scope.row.key" size="small" placeholder="请输入参数key" />
            </template>
          </el-table-column>
          <el-table-column align="center" prop="value" label="参数值">
            <template #default="scope">
              <el-input v-model="scope.row.value" size="small" placeholder="请输入参数值" />
            </template>
          </el-table-column>
      ...
      <div class="mb-2 mt-2">
        <div class="flex justify-between items-center mb-4">
          ...
          <div class="flex items-center gap-2">
            ...
            <el-tooltip content="点击查看按钮权限配置文档" placement="top" effect="light">
              <el-icon class="cursor-pointer text-blue-500 hover:text-blue-700"
                @click="toDoc('https://www.rust-vue-admin.com/guide/web/button-auth.html')">
                <QuestionFilled />
              </el-icon>
            </el-tooltip>
          </div>
        ...
        <el-table :data="form.menuBtn" style="width: 100%" class="button-table">
          <el-table-column align="center" prop="name" label="按钮名称" width="150">
            <template #default="scope">
              <el-input v-model="scope.row.name" size="small" placeholder="请输入按钮名称" />
            </template>
          </el-table-column>
          <el-table-column align="center" prop="desc" label="备注">
            <template #default="scope">
              <el-input v-model="scope.row.desc" size="small" placeholder="请输入按钮备注" />
            </template>
          </el-table-column>
          <el-table-column align="center" label="操作" width="100">
            <template #default="scope">
    ...
    <!-- 分配给角色抽屉 -->
    <el-drawer v-model="assignRoleDrawerVisible" :size="appStore.drawerSize" :show-close="false" destroy-on-close>
      <template #header>
        <div class="flex justify-between items-center">
      ...
      <el-tree ref="roleTreeRef" v-loading="assignRoleLoading" :data="authorityTreeData"
        :props="{ label: 'authorityName', children: 'children', disabled: isRoleDisabled }" node-key="authorityId"
